...
class AGQADataset(Dataset):
    def __init__(
        ...
        #TODO:set the frames
        self.numframes=16
        self.data = pd.read_json(args.data_path)
        self.candidate_answer = json.load(open(args.answer_path))
    ...
    def get_clip_txt_embedding(self, question):
        ...
        question=self.clip_tokenizer(question)["input_ids"]
        question_clip = self.clip.get_text_features(question)
    ...
    def get_formatted_prompt(self,question,answers,answer):
        answers=answers.split(" ")
        prompt=f"Below is a question and {len(answers)} options. Choose the correct answer from the options.\n"+question+"\n"
        for answer in answers:
            prompt=prompt+answer+" "
        prompt=prompt+"."
        return f"<image>User: {prompt} GPT:<answer>"+answer+"<|endofchunk|>"
    def __getitem__(self, index):
        cur_sample = self.data.iloc[index]
        qid = str(cur_sample['question_id'])
        #TODO:change vid to url
        # print("0OUEP-8098")
        # print(self.candidate_answer["0OUEP-8098"])
        ...
        video_url=self.video_path+"/"+raw_vid_id+".mp4"
        # print(video_url)

        # self.candidate_answer["0OUEP-8098"]=[]
        #TODO:查明为什么会不存在
        try:
            answer_candidates=' '.join(self.candidate_answer[question_id])
            # print(len(self.candidate_answer[question_id]))
        except KeyError as e:
            answer_supply=["doorway", "shoe", "blanket", "clothes", "door", "closet", "doorknob", "pillow", "table", "dish", "doorway", "shoe", "blanket", "clothes", "door", "closet", "doorknob", "pillow", "table", "dish", "box", "bag", "floor", "food", "paper", "bed", "light", "broom", "phone", "window"]
            if answer_txt in answer_supply:
                answer_candidates=' '.join(answer_supply)
            else:
                answer_supply[7]=answer_txt
                answer_candidates=' '.join(answer_supply)
            # print(answer_txt)
            # print(question_id)
        video_frames=self.extract_frames(video_path=video_url)
        video_embedds = self.image_processor.preprocess(video_frames, return_tensors="pt")["pixel_values"].unsqueeze(0)
        return {
            "video_embedds": video_embedds,
            # "question_clip": self.get_clip_txt_embedding(question=question_txt),

            "question_txt": question_txt,
            "type": type,
            "answer_txt": answer_txt,
            "prompts":self.get_formatted_prompt(question=question_txt,answers=answer_candidates,answer=answer_txt)
        }
