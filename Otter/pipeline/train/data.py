...
from pipeline.mimicit_utils.mimicit_dataset import MimicitDataset,AGQADataset
...
def get_agqa_dataset(args, image_processor, tokenizer, epoch=0, floor=False):
    agqadataset=AGQADataset(args,image_processor=image_processor)
    '''
    num_samples        
    '''
    # args.batch_size=1
    ...
    # num_batches = round_fn(num_samples / global_batch_size)  # 2
    # num_samples = num_batches * global_batch_size  # 8
    # sampler = RandomSampler(agqadataset, replacement=True, num_samples=num_samples)
    # if args.distributed_type == "DEEPSPEED" or args.distributed_type == "MULTI_GPU":
    #     sampler = DistributedProxySampler(sampler, num_replicas=args.world_size, rank=args.rank)
    dataloader = torch.utils.data.DataLoader(
        agqadataset,
        ...
        pin_memory=True,
        drop_last=True,
        collate_fn=agqadataset.collate,
    )
    return dataloader
...
def get_dataset_fn(dataset_type):
    ...
    elif dataset_type == "agqa":
        return get_agqa_dataset
    else:
