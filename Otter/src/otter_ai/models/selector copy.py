...
class MySelector3(nn.Module):
    def __init__(self,device,vision_dim=1024,qdim=512,hdim=1024,numc=4,nump=256,numf=16):
        """
    ...
    def forward(self, question_text,image_embeds,video_cls):
        ...
        question_clip=clip.tokenize(question_text)#(self.bz,numtext)
        question_clip2=self.tokenizer.tokenize(question_text)
        question_clip=question_clip.to(self.device)#(self.bz,numtext)
        q_feat, _ = self.get_clip_txt_embedding(question_clip)#(self.bz,qdim,1)

        #TODO:select执行 直接利用ista
        '''
        ista
        ...
        q_feat_tmp = q_feat.unsqueeze(dim=1).repeat(1, selected_patches.shape[1], 1, 1)  # [bs, topk * numf, num_obj, dim] [bs, topk * numf, dim, 1]
        q_feat_tmp = q_feat_tmp.view(-1, q_feat_tmp.shape[-2], q_feat_tmp.shape[-1])  # [bs * topk * numf, num_obj, dim] [bs * topk * numf, dim ,1]
        ...
        q_feat_tmp = q_feat.unsqueeze(dim=1).repeat(self.bz, selected_segs.shape[1], 1, 1)  # [bz, topk * numf, hdim, 1]
