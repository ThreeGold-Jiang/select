    ...
    def forward(self, x: torch.Tensor, latents: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            ...
            latent (torch.Tensor): latent features
                shape (b, T, n2, D)
        """
        x = self.norm_media(x)#?#(3,1,4096,1024)
        residual_latents = latents#(3,1,64,1024)
        latents = self.norm_latents(latents)

        h = self.heads

        q = self.to_q(latents)#(3,1,64,512)
        kv_input = torch.cat((x, latents), dim=-2)#(3,1,4160,1024)
        k, v = self.to_kv(kv_input).chunk(2, dim=-1)#(3,1,4096,512)
        q = rearrange(q, "b t n (h d) -> b h t n d", h=h)#(3,8,1,64,64)
        k = rearrange(k, "b t n (h d) -> b h t n d", h=h)#(3,8,1,4160,64)
        v = rearrange(v, "b t n (h d) -> b h t n d", h=h)#(3,8,1,4160,64)
        q = q * self.scale

        # attention
        sim = torch.einsum("... i d, ... j d  -> ... i j", q, k)#(3,8,1,64,4160)
        sim = sim - sim.amax(dim=-1, keepdim=True).detach()#(3,8,1,64,4160)
        attn = sim.softmax(dim=-1)#(3,8,1,64,4160)

        out = torch.einsum("... i j, ... j d -> ... i d", attn, v)#(3,8,1,64,64)
        out = rearrange(out, "b h t n d -> b t n (h d)", h=h)#(3,1,64,512)
        out = self.to_out(out) + residual_latents
        residual_out = out#(3,1,64,1024)
...
class OtterPerceiverResampler(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #self.frame_embs 128x1024
        if exists(self.frame_embs):
            frame_embs = repeat(self.frame_embs[:F], "F d -> b T F v d", b=b, T=T, v=v)#(3,1,16,256,1024)
            x = x + frame_embs
        x = rearrange(x, "b T F v d -> b T (F v) d")  # flatten the frame and spatial dimensions
        if exists(self.media_time_embs):
            x = x + self.media_time_embs[:T]
        # blocks
        latents = repeat(self.latents, "n d -> b T n d", b=b, T=T)#(3,1,64,1024)
        for block in self.layers:
            latents = block(x, latents)
        return self.norm(latents)#(3,1,64,1024)
...
class OtterForConditionalGeneration(OtterPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if use_cached_vision_x:
            ...
            # Case: do not use caching (i.e. this is a standard forward pass);
            if selector!=None:
                self._encode_vision_x(vision_x=vision_x,selector=selector,question_txt=question_txt)                 
            self._encode_vision_x(vision_x=vision_x)
    ...
    def _encode_vision_x(self, vision_x: torch.Tensor,selector=None,question_txt=""):
        """
        Compute media tokens from vision input by passing it through vision encoder and conditioning language model.
        Args:
            vision_x (torch.Tensor): Vision input
        ...
        b, T, F = vision_x.shape[:3]
        #(3,1,32,3,224,224)
        vision_x = rearrange(vision_x, "b T F c h w -> (b T F) c h w")#(96,224,224)
        with torch.no_grad():
            #THREEGOLD CHANGE
            vision_x = self.vision_encoder(vision_x)[0][:, :, :]#(96,256,1024)
            # print(vision_x.is_contiguous())
            vision_cls=vision_x[:, 0, :].unsqueeze(1)#(96,1,1024)
            # print(vision_x.is_contiguous())
            # print(vision_x.stride())

            vision_x=vision_x[:,1:,:]
        # print(vision_x.is_contiguous())
        # print(vision_x.stride())
        vision_x = rearrange(vision_x, "(b T F) v d -> b T F v d", b=b, T=T, F=F)#(3,1,32,256,1024)
        # print(vision_x.is_contiguous())
        vision_cls=rearrange(vision_cls,"(b T F) v d -> b T F v d", b=b, T=T, F=F)#(3,1,32,1,1024)
        if selector!=None:
            vision_x=selector(question_txt,vision_x,vision_cls)            
        vision_x = self.perceiver(vision_x)  # reshapes to (b, T, n, d)
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        num_beams = generate_kwargs.get("num_beams", 3)
        print(num_beams)
        if num_beams > 1:
            vision_x = vision_x.repeat_interleave(num_beams, dim=0)#(3,1,32,3,224,224)
        self._encode_vision_x(vision_x=vision_x,selector=selector,question_txt=question_txt)
